...
| 무료 운영 우선 | Cloudflare Pages/Functions/D1, Google OAuth/Sheets/Drive/Docs API, Gemini API, GitHub/Codespaces 모두 무료 한도 안에서 동작하는 것을 기본 전제로 설계한다. 무료 한도를 넘기면 자동으로 유료로 전환하지 않고 요청을 제한하거나 관리자에게 알린다(18절 참고). |
...
| `userId` | `user`를 가리키는 D1 저장용 식별자(Google 계정 고유 ID). 이메일은 별도 컬럼으로 참조용으로만 둔다. | 최초 로그인 시 | D1에 저장되는 필드명 기준. 18.2절 "D1 최소 사용"의 저장 대상 중 하나. |
...
**무료 운영 관점**: `Pages`가 서빙하는 완전 정적 화면(예: `/`)은 `Functions`를 전혀 호출하지 않는다. `Functions`와 Google API 사이에는 클라이언트/서버 캐시가 개입해 동일 데이터의 반복 조회를 줄인다. `ConnStore`(D1)는 연결·설치 메타데이터만 담아 최소한으로 유지한다. 세부 정책은 18절 참고.
...
이 절의 로그인/설치 스코프 분리, `drive.file` 우선 검토, 연결 해제 설계는 18.4절(Google OAuth 최소 권한)의 무료 운영 정책과 동일한 결정이다 — 18.4절은 이 절의 요약이며 별도 결정을 추가하지 않는다.
...
| B. Cloudflare의 사용자별 암호화 저장소 | Cloudflare Functions/Workers 환경에서 tenant별로 키를 암호화해 저장(값 자체는 서버측 비밀키로 암호화). **D1은 아니다** — D1은 18.2절 "D1 최소 사용" 원칙에 따라 연결·설치 메타데이터 9종만 저장하며 Gemini Key는 포함하지 않는다. 별도의 Cloudflare 암호화 저장소(예: D1과 분리된 KV 네임스페이스)를 사용한다. | **v1 채택안.** 모든 API 중계가 이미 Cloudflare Functions를 거치므로 저장 위치와 사용 위치가 일치한다. Apps Script 존치 여부와 무관하게 동작한다. |
...
이 절은 Key의 저장·호출 메커니즘만 다룬다. 어떤 모델을 기본값으로 둘지, 유료 모델을 어떻게 제외할지, 일일 호출 횟수를 어떻게 제한할지는 18.5절(Gemini 무료 우선)에서 정의한다.
...
- **토큰 해시 저장**: **tenant 자신의 `보호자동의` 시트**(D1이 아님)에는 token 원문이 아니라 해시값만 저장하고, 사용자가 링크로 전달받은 원문 token과 대조(해시 비교)하는 방식으로 검증한다. legacy는 원문 token을 시트에 그대로 저장했다(`google-data-inventory.md` 1절 "동의토큰" 컬럼) — v1은 이를 해시 저장으로 전환한다. 동의 건수는 tenant마다 많아질 수 있는 데이터이므로 D1이 아니라 tenant Sheet에 두어 18.2절의 "D1 최소 사용" 원칙을 지킨다.
...
**무료 운영 관점**: A로 분류된 기능이 많을수록 Cloudflare Functions 호출 수가 늘어나므로, 정적 화면은 애초에 A 경로를 타지 않도록 한다(18.1절). Apps Script(B/C 분류)는 Google 계정의 자체 실행 할당량을 쓰므로 Cloudflare 무료 한도와는 별개지만, Apps Script 자체도 일일 실행시간 등 무료 한도가 있다(18.9절 비교표 참고).
...
- **플랫폼(D1) 측에 반드시 저장해야 하는 최소 메타데이터**: 18.2절 "D1 최소 사용"에서 정의한 9개 필드가 전부이며, 그 이상을 D1에 추가하지 않는다.
  - `userId`
  - `tenantId`
  - `installationId`
  - 설치 상태
  ...
  - 암호화된 OAuth token
  - 공개 링크 식별자(`schoolPublicId` 등)
  - 생성일·수정일
- **D1에 두지 않고 다른 곳에 두는 것으로 정리한 항목**(이전 안에서 조정):
  - 암호화된 Gemini API Key → D1이 아닌 별도의 Cloudflare 암호화 저장소(10절, 18.5절)
  - tenant 설정값(학교명, 활성화된 모듈 등) → tenant 자신의 Spreadsheet `설정` 탭(7절). 플랫폼이 별도로 들고 있을 필요가 없어 애초에 중복 저장하지 않는다.
  - 플랫폼 차원의 감사로그 → v1에서는 D1에 별도 로그 테이블을 두지 않는다(D1 최소화 원칙, 18.2절). 필요해지면 Cloudflare의 로그/Analytics 도구 등 D1 외부 수단을 검토한다(17절 미결정 사항에 추가).
...
Milestone 1은 18.1절의 "정적 화면은 Functions를 호출하지 않는다" 원칙을 이미 따른다(`/` 홈 화면은 API 호출 없음). 세션/설치 정보를 D1으로 옮기는 시점부터는 18.2절의 9개 필드 스키마를 그대로 적용한다.
...
- **사용량 70% 경고의 실제 집계 방식**(18.1절): Cloudflare Analytics/GraphQL API를 조회할지, 자체 카운터를 D1 외부(예: 경량 KV)에 둘지 결정 필요.
- **Gemini 무료 지원 모델 목록과 기본값의 구체 확정**(18.5절): Google이 모델 구성을 자주 바꾸므로, 목록을 코드 상수로 고정할지 주기적으로 갱신할 프로세스를 둘지 결정 필요.
- **사용자별 일일 Gemini 호출 제한의 기본 수치**(18.5절): 상한값과, tenant별로 다르게 설정할 수 있게 할지 여부.
- **감사로그를 D1 외부 어디에 둘지, 또는 v1에서 아예 생략할지**(14절, 18.2절).
- **무료 한도 초과 시 "읽기 전용 전환"의 구체 UX**(18.8절): 어떤 화면에 어떤 문구/배너를 어떤 조건으로 표시할지.
- **재시도 가능한 작업 큐/임시저장 구조의 구체 기술 선택**(18.8절): Cloudflare Queues 등은 무료 플랜 포함 여부가 다를 수 있어 별도 검토가 필요하다.

## 18. 무료 운영 우선 정책

이 절은 v1을 결제 계정 없이(또는 결제 계정을 연결하더라도 과금 없이) 운영할 수 있도록 하는 정책을 정의한다. 1절의 "무료 운영 우선" 원칙의 구체화이며, 9~14절의 관련 설계를 이 원칙 기준으로 재확인한다.

### 18.1 Cloudflare 무료 플랜 우선

- Cloudflare Pages(정적 호스팅)와 Pages Functions(API)는 무료 플랜 한도 안에서 운영하는 것을 기본 전제로 한다.
- **정적 화면은 Functions를 호출하지 않는다.** `/`처럼 로그인·데이터 조회가 필요 없는 화면은 `/api/*`를 전혀 호출하지 않고 정적 자산만으로 렌더링한다.
- **동일 데이터 반복 요청 금지**: 같은 요청 안에서 이미 가져온 데이터를 다시 조회하지 않도록 클라이언트 캐시(브라우저 메모리/세션 단위)를 두고, Functions 쪽에도 짧은 TTL의 서버 캐시를 두어 같은 데이터를 반복해서 Google API까지 내려보내지 않는다.
- **사용량 모니터링 구조 준비**: Functions 호출 수(또는 D1/Google API 호출 수)가 무료 한도의 70%에 도달하면 관리자(`/settings` 또는 `/app`)에 경고를 표시할 수 있는 구조를 준비한다. v1에서는 이 구조(측정 지점과 표시 위치)만 설계하고, 실제 집계 로직 구현은 후속 마일스톤 과제다(17절 미결정).
- **무료 한도 초과 시 자동 유료 전환 금지**: 어떤 서비스도 한도 초과 시 자동으로 유료 플랜으로 전환되지 않는다. 유료 전환은 항상 사람(관리자)의 명시적 조작을 거친다(18.6절).

### 18.2 D1 최소 사용

D1에는 아래 9개 필드에 해당하는 메타데이터**만** 저장한다.

| 필드 | 설명 |
|---|---|
| `userId` | 로그인한 Google 계정의 고유 식별자 |
| `tenantId` | 학교 작업공간 식별자 |
| `installationId` | 설치 단위 식별자(`tenantId`와 1:1, 2절 참고) |
| 설치 상태 | 설치 진행 중/완료 등 상태값 |
| Google Spreadsheet ID | tenant의 데이터 Spreadsheet 참조(서버측 전용, 9·5절) |
| Google Drive Folder ID | tenant의 Drive 루트 폴더 참조(서버측 전용) |
| 암호화된 OAuth token | refresh token(및 필요 시 access token), 암호화 저장(9절) |
| 공개 링크 식별자 | `schoolPublicId` 등 공개 페이지 식별에 쓰는 값 |
| 생성일·수정일 | 레코드 생성/갱신 시각 |
...
위 5가지는 물론이고, 14절에서 정리했듯 Gemini API Key(별도 Cloudflare 암호화 저장소), tenant 설정값(tenant 자신의 Spreadsheet `설정` 탭), 감사로그(v1에서는 D1에 두지 않음)도 D1에 두지 않는다. D1은 "누가 어떤 Spreadsheet/Folder를 갖고 있는가"라는 연결 정보만 갖고, 실제 데이터는 항상 사용자의 Google Sheets/Drive에만 존재한다는 1절/14절 원칙을 D1 스키마 수준에서 강제하는 것이 이 절의 목적이다.

### 18.3 Google API 호출 최소화

- **페이지 이동마다 전체 시트를 다시 읽지 않는다.** 화면을 옮길 때마다 Spreadsheet 전체를 재조회하지 않고, 그 화면에 필요한 범위/탭만 조회한다(11-A절의 지연 로딩과 동일한 방향).
- **batchGet/batchUpdate 우선**: 여러 범위를 개별 요청으로 나누지 않고 Google Sheets API의 배치 엔드포인트로 묶어 호출 횟수를 줄인다.
- **메뉴별 lazy loading**: 11-A절과 동일 — 진입 화면은 최소 데이터만, 나머지는 실제로 그 메뉴에 들어갈 때 로드한다.
- **브라우저 세션 캐시 적용**: 같은 로그인 세션 안에서 이미 읽은 데이터는 새로고침 없이는 다시 요청하지 않는다.
- **변경된 데이터만 다시 조회**: 저장/수정 이후에는 변경된 탭·범위만 다시 읽고, 관련 없는 탭은 캐시를 그대로 유지한다.
- **429 응답 시 exponential backoff 적용**: Google API가 429(Too Many Requests)를 반환하면 즉시 재시도하지 않고 지수적으로 대기 시간을 늘리며 재시도 횟수에 상한을 둔다.
- **Drive 파일 목록 전체 조회 금지**: `files.list`처럼 비용이 큰 목록 조회 대신, 이미 D1(18.2절)에 저장해 둔 특정 파일/폴더 ID로 직접 접근한다.

### 18.4 Google OAuth 최소 권한

이 항목의 전체 설계는 9절에 있으며, 이 절은 무료/비용 관점에서 동일한 결정을 다시 확인한다.

- 최초 로그인에서는 `openid email profile`만 요청하고, Drive/Sheets 권한은 설치 시작 시점에 별도로 요청한다(9절 "로그인 권한과 Drive/Sheets 권한 분리").
- 가능한 경우 `drive` 전체 범위 대신 `drive.file` 범위를 우선 검토한다(9절 "최소 권한 원칙").
- 사용자가 `/settings`에서 언제든 연결을 해제할 수 있도록 구현한다(9절 "사용자 연결 해제").
- 연결 해제 후에도 사용자의 Drive/Sheets 데이터는 삭제하지 않고 그대로 사용자 소유로 유지한다(14절).

### 18.5 Gemini 무료 우선

- 각 사용자가 자신의 Gemini API Key를 직접 입력해 등록한다(10절 — 저장/호출 메커니즘은 10절, 이 절은 모델·한도 정책만 다룬다).
- **무료 지원 모델을 기본값으로 사용**한다. 모델 선택 드롭다운의 기본 선택값은 항상 무료 등급으로 호출 가능한 모델이다.
- **유료 전용 모델은 기본 목록에서 제외**한다. 선택 가능한 모델 목록 자체를 무료 등급이 있는 모델로 제한하는 것을 기본으로 하고, 유료 전용 모델을 노출하려면 별도의 명시적 조작이 필요하다(18.6절 "유료 기능은 관리자가 직접 활성화").
- **모델별 무료 여부는 설정값으로 관리**한다. 이 값은 tenant마다 달라지는 데이터가 아니라 플랫폼 전역 설정(코드 상수 또는 배포 시 갱신하는 설정 목록)이므로 D1이나 tenant Sheet에 저장하지 않는다.
- **호출 전 무료/유료 가능성을 사용자에게 안내**한다. 유료로 청구될 수 있는 모델을 선택하면 실행 전에 경고를 표시한다.
- **일일 사용자별 AI 호출 제한을 설정 가능하게 준비**한다. 한도 값 자체는 tenant마다 다를 수 있으므로 tenant Spreadsheet의 `설정` 탭에 둘 수 있게 하고, "오늘 호출한 횟수" 같은 카운터는 D1에 누적하지 않고 짧은 TTL의 캐시로 관리해 18.2절의 D1 최소화 원칙을 지킨다. 기본 상한값은 17절 미결정 사항으로 남겨둔다.
- **운영자 API Key를 공용으로 사용하지 않는다.** 플랫폼이 자체 Gemini Key로 여러 사용자의 호출을 대신 처리하는 경로는 두지 않는다 — 모든 호출은 그 사용자 본인이 등록한 Key로만 이뤄진다.

### 18.6 비용 방지 장치

- Cloudflare 유료 플랜으로의 자동 업그레이드를 금지한다.
- Google Cloud 결제 계정 연결을 Milestone 1의 필수 사항으로 두지 않는다(`google-oauth-setup.md` 5절 — 로그인 전용 스코프는 별도 API 활성화가 필요 없다).
- 유료 전용 Google/기타 API의 활성화를 금지한다.
- 결제 계정이 없어도 동작하는 기능부터 구현한다(16절 로드맵 순서 — 로그인/설치/CRUD가 Gemini보다 먼저이며, Gemini도 사용자 본인 Key로만 동작).
- 비용이 발생할 수 있는 기능(예: 유료 모델 호출)은 코드와 UI 양쪽에 명시적 경고를 추가한다.
- 향후 유료 기능은 기본값을 꺼짐(off)으로 두고, 관리자가 직접 활성화해야만 동작하도록 설계한다(feature flag 방식).

### 18.7 Codespaces 비용 절감

- 개발 중이 아닐 때는 Codespace를 중지하도록 README 등에서 안내한다(문서화는 후속 작업 — 이 문서 갱신 시점에는 정책만 정의).
- 대형 머신 스펙을 사용하지 않고, 기본 2코어 환경을 우선한다.
- 불필요한 캐시나 대용량 파일을 저장소에 커밋하지 않는다(`node_modules`, `dist` 등은 이미 `.gitignore` 대상).
- `legacy/` 압축파일과 빌드 생성물의 크기를 주기적으로 점검해 저장소가 불필요하게 커지지 않도록 관리한다.

### 18.8 무료 한도 초과 시 동작

- 무료 한도를 초과하면 자동 결제 대신 **요청을 차단하거나 읽기 전용으로 전환**한다.
- 한도 초과 사실을 관리자에게 안내한다(`/app` 또는 `/settings`에 표시하는 것을 원칙으로 하되, 구체 UX는 17절 미결정).
- 어떤 경우에도 **사용자의 Google 데이터(Sheets/Drive)를 손상시키지 않는다** — 쓰기 도중 한도를 초과하면 부분 반영 대신 실패로 처리하고 사용자에게 재시도를 안내한다.
- 재시도 가능한 작업 큐 또는 임시저장 구조 도입 여부를 검토한다(구체 기술 선택은 17절 미결정).

### 18.9 무료 운영 가능성 표

> 아래 무료 한도 수치는 각 서비스 제공자의 정책 변경에 따라 달라질 수 있다. 실제 배포 전에 Cloudflare/Google 공식 문서 기준으로 재확인이 필요하다.

| 서비스 | 무료 여부 | 주요 무료 제한 | 한도 초과 시 결과 | 현재 프로젝트의 대응책 |
|---|---|---|---|---|
| GitHub | 무료(개인 계정 기준) | 저장소 용량 권장 상한, 파일당 100MB, Actions 무료 실행 시간(계정 유형별 상이) | 대용량 파일 push 거부, Actions 중단 | `node_modules`/`dist`/빌드 산출물을 `.gitignore`로 제외, `legacy/` 크기 관리(18.7절) |
| Codespaces | 월 무료 사용량 포함(계정 유형별 상이) | 월 사용 시간, 저장 용량 상한 | 결제 수단 등록 시 초과분 과금, 미등록 시 신규 생성 차단 | 미사용 시 중지 안내, 기본 2코어 사용(18.7절) |
| Cloudflare Pages | 무료 | 월 빌드 횟수 제한, 동시 빌드 1개 | 이번 달 빌드 대기/실패 | 불필요한 재배포 최소화 |
| Pages Functions | 무료(Workers 무료 한도 공유) | 일일 요청 수 상한, 요청당 CPU 시간 제한 | 초과 요청 오류 응답 | 정적 화면 API 미호출, 캐시로 호출 수 절감, 70% 사용량 경고(18.1절) |
| D1 | 무료 플랜 존재 | 저장 용량 상한, 일일 읽기/쓰기 행 수 상한 | 초과 시 쓰기 거부/오류 | 18.2절 9개 필드로 스키마 최소화, 학생 데이터 저장 금지 |
| Google OAuth | 무료(사용량 과금 없음) | 미검증 앱은 등록된 테스트 사용자만 로그인 가능 | 테스트 사용자 목록 밖 계정은 로그인 거부 | 파일럿 단계는 테스트 사용자로 운영(`google-oauth-setup.md` 6절), 확장 시 Google 검증 절차 진행 |
| Sheets API | 무료(할당량 기반) | 사용자·프로젝트별 분당 요청 수 제한 | 429 오류 | batchGet/batchUpdate, 캐시, exponential backoff(18.3절) |
| Drive API | 무료(할당량 기반) | 분당 요청 수 제한, 목록 조회 비용이 상대적으로 큼 | 429 오류 | 전체 목록 조회 대신 ID 직접 접근(18.3절) |
| Docs API(Apps Script 경유) | 무료 | Apps Script 자체의 일일 실행시간/URL Fetch 호출 수 한도(계정 유형별 상이) | 스크립트 실행 실패 | PDF 생성은 사용자 이벤트당 1회 호출로 제한(12절 C분류) |
| Gemini API | 모델별로 상이(무료 등급 모델 존재, 일부는 유료 전용) | 무료 등급 모델의 분당/일당 요청 수 제한 | 429 또는 유료 결제 필요 | 무료 모델 기본값, 유료 모델 목록 제외, 사용자별 일일 호출 제한 준비(18.5절) |
| Apps Script | 무료(개인 계정 기준, Workspace는 더 높은 한도) | 실행당 시간 제한, 일일 총 실행 시간, URL Fetch 호출 수 | 실행 중단/오류 | 무거운 로직은 Cloudflare Functions로 우선 이전(12절 A분류), Apps Script는 Docs/PDF 등 최소 기능만 유지 |
